...
const MessageBubble = ({ 
  ...
  if (msg.is_deleted) {
    return (
      <div className="flex w-full mb-1 justify-start">
        <div className="flex items-center gap-2 text-muted-foreground text-xs italic py-2 px-4 bg-muted/30 rounded-lg">
          <Trash2 className="w-3 h-3" />
          <span>Message deleted</span>
        </div>
      </div>
...
// --- COMPONENT: COMMUNITY INFO DIALOG ---
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] max-h-[85vh] flex flex-col">
        ...
        {community.description && (
          <div className="py-3 border-b bg-muted/20 p-3 rounded-lg">
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {m.role === 'admin' && (
                            <Badge className="text-[10px] h-5 bg-amber-500 hover:bg-amber-600">
...
// --- COMPONENT: COMMUNITY SETTINGS DIALOG ---
...
export default function Messages() {
  // Fix: Check if user exists before rendering logic
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const scrollRef = useRef<HTMLDivElement | null>(null);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  ...
  // 3. Friends List (For creating new DMs)
  const { data: friends = [] } = useQuery({
    queryKey: ['my_friends', user?.id],
    queryFn: async () => {
      if (!user) return [];
  ...
  // --- SAFETY CHECK ---
  // If auth is rendering, show spinner. 
  // NOTE: Removed strict auth check here to let MainLayout handle redirects, 
  // ensuring the component mounts correctly.
  if (!user) return <div className="flex h-full items-center justify-center"><Loader2 className="animate-spin" /></div>;
  ...
  if (selectedChat) {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-background flex flex-col h-[100dvh] animate-in slide-in-from-right-10 duration-200">
              ...
              {isComm ? (
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  {selectedChat.my_role === 'admin' && (
                    <Badge variant="secondary" className="ml-1 text-[10px] px-1 py-0">Admin</Badge>
                  )}
                </p>
              ) : (
        ...
        {/* Input Area */}
        <div className="p-3 border-t bg-background shrink-0 safe-area-bottom">
          {canType ? (
            <div className="flex flex-col gap-3">
              {imagePreview && (
                <div className="relative w-24 h-24 bg-muted rounded-xl overflow-hidden border-2 border-primary/20 animate-in fade-in zoom-in">
                  <img 
                    src={imagePreview} 
              ...
              <div className="flex items-end gap-2">
                ...
                <Button 
                  ...
                  className="rounded-full h-11 w-11 shadow-md shrink-0 gradient-primary text-white"
                >
            ...
            <Button 
              className="w-full rounded-xl shadow-md gradient-primary text-white" 
  ...
  return (
    <div className="flex flex-col h-full pb-20 bg-background overflow-hidden">
      <div className="container max-w-4xl mx-auto pt-6 px-4 flex-1 flex flex-col min-h-0">
        <div className="flex items-center justify-between mb-6 px-1 shrink-0">
          ...
          <Button 
            size="icon" 
            className="rounded-full shadow-lg gradient-primary text-white h-10 w-10" 
        ...
        >
          ...
          <TabsContent value="dm" className="space-y-2 animate-in fade-in-50 px-1 mt-0 flex-1 overflow-y-auto pb-4">
            {loadingDMs ? (
          ...
          <TabsContent value="community" className="space-y-2 animate-in fade-in-50 px-1 mt-0 flex-1 overflow-y-auto pb-4">
            {loadingComms ? (
              <div className="p-8 text-center text-muted-foreground">
                <Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" />
                  ...
                  >
                    ...
                    <Button 
                      ...
                      className={comm.my_role !== 'none' 
                        ...
                        : "rounded-full px-6 shadow-md gradient-primary text-white"
                      }
