...
export default function Messages() {
  const { user } = useAuth(); // Ensure your AuthContext exposes 'user'
  const queryClient = useQueryClient();
      ...
      data?.forEach((msg: any) => {
        ...
        if (!map.has(partner.user_id)) {
          map.set(partner.user_id, {
            ...
            last_msg: msg.content || 'Attachment', 
            time: msg.created_at
          });
        }
      });
  ...
  const { data: friends = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data?.map((f: any) => {
        ...
        return { id: profile.user_id, name: profile.display_name || 'User', avatar: profile.avatar_url };
      }) || [];
    },
    enabled: isNewChatOpen && !!user
  });
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (selectedChat.type === 'dm') {
        ...
        data = res.data?.map((m: any) => ({ ...m, sender_name: m.sender?.display_name || 'User', sender_avatar: m.sender?.avatar_url }));
      }
  ...
  const sendMessage = useMutation({
    ...
    onSuccess: () => { setMessageInput(''); setImageFile(null); setImagePreview(null); queryClient.invalidateQueries({ queryKey: ['messages'] }); },
  ...
  if (!user) return <div className="flex items-center justify-center h-screen"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>;
    ...
    return (
      <div className="fixed inset-0 z-50 bg-background flex flex-col h-screen">
          ...
          <div className="flex flex-col justify-end min-h-full pb-2">
             {messages.length === 0 ? (
               <div className="flex-1 flex flex-col items-center justify-center opacity-50">
                 ...
                 <MessageBubble key={m.id} msg={m} prevMsg={i > 0 ? messages[i-1] : null} isComm={isComm} />
             ))}
          </div>
        </div>
        ...
        <div className="p-3 border-t bg-background shrink-0 pb-safe">
            ...
            <div className="flex flex-col gap-3">
              {imagePreview && (
                <div className="relative w-24 h-24 bg-muted rounded-xl overflow-hidden border">
                  ...
                  <button onClick={() => { setImageFile(null); setImagePreview(null); }} className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-0.5 hover:bg-black/70"><X className="w-4 h-4" /></button>
                </div>
              )}
              <div className="flex items-end gap-2">
                ...
                <div className="flex-1 relative">
                   <Textarea value={messageInput} onChange={(e) => setMessageInput(e.target.value)} placeholder="Message..." className="min-h-[44px] max-h-32 py-3 pr-10 resize-none rounded-2xl bg-muted/50 border-transparent focus:border-primary focus:bg-background transition-all" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage.mutate(); } }}/>
                </div>
          ...
          ) : <Button className="w-full rounded-xl shadow-md" onClick={() => joinCommunity.mutate(selectedChat.id)}>Join Community to Chat</Button>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container-mobile pt-6 min-h-screen flex flex-col pb-20 bg-background">
      <div className="flex items-center justify-between mb-6 px-1">
        <div><h1 className="text-3xl font-bold tracking-tight">Chats</h1><p className="text-muted-foreground text-sm font-medium mt-1">Connect with your circle</p></div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ChatMode)} className="w-full flex-1 flex flex-col">
        <div className="px-1 mb-4">
          <div className="bg-muted/50 p-1 rounded-xl flex"><TabsTrigger value="dm" className="flex-1 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all">Private</TabsTrigger><TabsTrigger value="community" className="flex-1 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all">Communities</TabsTrigger></div>
          <div className="relative mt-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Search conversation..." className="pl-10 bg-muted/30 border-transparent rounded-xl focus:bg-background focus:border-primary/20 transition-all" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
        </div>
        ...
        <TabsContent value="community" className="flex-1 space-y-2 animate-in fade-in-50 px-1">
          {loadingComms ? <div className="p-8 text-center text-muted-foreground"><Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" />Loading communities...</div> : commList.length === 0 ? (
             <div className="flex flex-col items-center justify-center h-64 text-center opacity-60"><Users className="w-12 h-12 mb-4 text-muted-foreground" /><p>No communities yet.</p><Button variant="link" onClick={() => setIsCreateCommunityOpen(true)}>Create one</Button></div>
          ) : (
             commList.filter((c: any) => c.name.toLowerCase().includes(searchQuery.toLowerCase())).map((comm: any) => (
               <div key={comm.id} className="flex items-center gap-4 p-3 hover:bg-muted/50 rounded-2xl transition-all border border-transparent hover:border-border/50">
                 ...
                 <Button size="sm" variant={comm.my_role !== 'none' ? "ghost" : "default"} className={comm.my_role !== 'none' ? "text-muted-foreground" : "rounded-full px-6 shadow-md"} onClick={comm.my_role !== 'none' ? () => setSelectedChat(comm) : () => joinCommunity.mutate(comm.id)}>{comm.my_role !== 'none' ? 'Open' : 'Join'}</Button>
               </div>
             ))
          )}
        </TabsContent>
