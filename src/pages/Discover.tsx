...
import { 
  ...
  Heart, Share2, Sparkles, Lock, RefreshCw, Search
} from "lucide-react";
...
// 2. RICH EMPTY STATE (The "Captivating" Placeholder)
      ...
      <div className="relative w-full h-full sm:max-w-md sm:h-[85vh] bg-black sm:rounded-2xl overflow-hidden flex flex-col border border-white/10 shadow-2xl">
        ...
        <div className="flex-1 flex items-center justify-center bg-black relative" onClick={next}>
          {current.media_type === 'video' ? (
             <video src={current.media_url} className="w-full h-full object-contain" autoPlay playsInline />
          ) : (
             <img src={current.media_url} className="w-full h-full object-contain" />
          )}
          
          {current.caption && (
             <div className="absolute bottom-24 w-full text-center px-6">
               <span className="bg-black/40 text-white px-4 py-2 rounded-xl backdrop-blur-md text-lg font-medium inline-block">
                 {current.caption}
               </span>
             </div>
          )}
        ...
        <div className="absolute bottom-0 w-full p-4 z-30 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex gap-3 pb-8">
          ...
          />
          <Button size="icon" variant="ghost" className="text-white rounded-full hover:bg-white/10" onClick={(e) => { e.stopPropagation(); setLiked(!liked); toast.success("Reaction sent ❤️"); }}>
  ...
  return (
    <div className="container-mobile py-4 space-y-6 pb-24">
      ...
      <div className="w-full overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4">
        {loading ? (
          <div className="flex gap-4"><div className="w-16 h-16 bg-muted rounded-full animate-pulse" /><div className="w-16 h-16 bg-muted rounded-full animate-pulse" /></div>
        ) : (
        ...
        <DialogContent className="sm:max-w-md bg-background/95 backdrop-blur-xl border-0">
          ...
          <div className="space-y-4 pt-2">
            ...
            <DialogFooter className="gap-2">
               ...
               <Button onClick={handleUpload} disabled={uploading} className="gradient-primary text-white">{uploading ? <Loader2 className="animate-spin" /> : 'Share to Story'}</Button>
            </DialogFooter>
          </div>
        </DialogContent>
      </Dialog>

      {/* MAIN TABS (Reordered: Communities -> Events -> For You) */}
      <div className="px-1">
        ...
        <Tabs defaultValue="communities">
          <TabsList className="grid w-full grid-cols-3 bg-muted/50 p-1 rounded-xl">
            ...
            <TabsTrigger value="foryou" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-600 data-[state=active]:to-blue-600 data-[state=active]:text-white rounded-lg">
               <Sparkles className="w-3 h-3 mr-1" /> For You
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="communities" className="mt-6 space-y-3 animate-in fade-in-50">
            {loading ? <FeedSkeleton /> : communities.length === 0 ? (
              <EmptyState 
                ...
                onAction={() => navigate('/app/messages')} // Redirects to messages where "Create" lives
              />
            ) : (
              communities.map(c => (
          ...
          <TabsContent value="events" className="mt-6 space-y-3 animate-in fade-in-50">
            {loading ? <FeedSkeleton /> : events.length === 0 ? (
               <EmptyState 
                 ...
                 desc="It's quiet... too quiet. Host a party!" 
                 action="Create Event"
            ...
            {!isPremium ? (
              <Card className="bg-gradient-to-br from-indigo-900 to-purple-900 border-0 text-white shadow-xl relative overflow-hidden">
                ...
                <CardContent className="flex flex-col items-center justify-center py-12 text-center relative z-10">
                  <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center mb-4 backdrop-blur-md shadow-inner border border-white/20">
                  ...
                  <p className="text-white/70 max-w-xs mb-6 text-sm leading-relaxed">
                    See events matched to your interests and friends' activity with an 80x boost in relevance.
                  </p>
                  <Button variant="secondary" className="font-bold shadow-lg hover:scale-105 transition-transform" onClick={() => navigate('/premium')}>
                    Upgrade to Premium
                  </Button>
                </CardContent>
              </Card>
               ...
               <EmptyState 
                 icon={RefreshCw} 
                 title="Analyzing..." 
                 desc="AI is learning your preferences. Check back soon." 
                 action="Refresh"
              ...
              smartFeed.map(e => (
                <Card key={e.id} className="overflow-hidden border-purple-200 dark:border-purple-900 shadow-sm hover:shadow-md transition-all">
                  ...
                  <CardContent className="p-4">
                    <h3 className="font-bold truncate text-lg">{e.name}</h3>
